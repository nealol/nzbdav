...
) : BaseStoreReadonlyCollection
{
    ...
    {
        var (dir, ctx, db, usenet, config, tracker) = (request.Name, httpContext, dbClient, usenetClient, configManager, activeStreamTracker);
        if (_currentPathParts.Length < DavItem.IdPrefixLength)
        {
            ...
            return new DatabaseStoreIdsCollection(dir, Path.Join(currentPath, dir), ctx, db, usenet, config, tracker);
        }

        var item = await dbClient.GetFileById(request.Name).ConfigureAwait(false);
        return item == null ? null : new DatabaseStoreIdFile(item, ctx, dbClient, usenet, config, tracker);
    }

    protected override async Task<IStoreItem[]> GetAllItemsAsync(CancellationToken cancellationToken)
    {
        var (ctx, db, usenet, config, tracker) = (httpContext, dbClient, usenetClient, configManager, activeStreamTracker);
        if (_currentPathParts.Length < DavItem.IdPrefixLength)
            return Alphabet
                .Select(x => x.ToString())
                .Select(x => new DatabaseStoreIdsCollection(x, Path.Join(currentPath, x), ctx, db, usenet, config, tracker))
        ...
        return (await dbClient.GetFilesByIdPrefix(idPrefix).ConfigureAwait(false))
            .Select(x => new DatabaseStoreIdFile(x, ctx, db, usenet, config, tracker))
            .Select(x => x as IStoreItem)
            .ToArray();
    }
}